...
while ($row = $resultado->fetch_assoc()) {
    $id = $row['id'];
    echo ($row['id'] . " - " . $row['name'] . " - " . $row['comment'] . " - " . $row['email'] . " - " . $row['post_id'] . " - " . $row['created_at'] . " - " . $row['status'] . " <button onclick=\"location.href='../System/C_DeleteComment.php?id=$id'\">Borrar</button> " . " <button onclick=\"location.href='V_formComment.php?id=$id'\">Editar</button>");
